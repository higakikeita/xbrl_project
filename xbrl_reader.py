...
[ドキュメント]def read_lines(file_name:str, encoding='utf-8'):
...
[ドキュメント]def read_csv_file(file_name:str, encoding='utf-8'):
...
[ドキュメント]def find(search_iteration):
    """リスト内で条件に合う要素を返す。
    """
    try:
...
[ドキュメント]def inc_key_cnt(dic: dict, key):
...
[ドキュメント]def log_dict_cnt(inf, name, dic: dict):
    """辞書の値をログファイルに書く。
    """
...
[ドキュメント]def findObj(v: dict, key, val):
    """指定したキーの値を返す。
    """
...
[ドキュメント]    def setLabel(self, role, text):
        if role == label_role:
            self.label = text
...
[ドキュメント]class XbrlNode:
    def __init__(self):
        pass

[ドキュメント]    def set_schema(self, schema):
        """schemaをセットする。
...
[ドキュメント]class ContextNode(XbrlNode):
...
[ドキュメント]class Dimension(XbrlNode):
    """ディメンション軸
    """
...
[ドキュメント]class MyJSONEncoder(JSONEncoder):
    # False positive for E0202 (method-hidden) 
    #   https://github.com/PyCQA/pylint/issues/414
[ドキュメント]    def default(self, o): # pylint: disable=E0202
...
[ドキュメント]def split_uri_name(text):
...
[ドキュメント]def getAttribs(el: ET.Element) -> Dict[str, str]:
    """属性のラベルと値の辞書を作って返す。
    """
    # 属性のラベルと値の辞書
    attr: Dict[str, str] = {}
...
[ドキュメント]def parseElement(el: ET.Element):
...
[ドキュメント]def getSchemaElementNsName(inf, text) -> SchemaElement:
    # 名前空間の接頭辞と要素名に分離する。
...
[ドキュメント]def ReadLabel(el, xsd_dic, loc_dic, resource_dic):
    """名称リンクファイルの内容を読む。
    """
    if el.tag[0] == '{':
...
[ドキュメント]def setChildren(inf, ctx: ContextNode):
...
[ドキュメント]def readCalcArcs(xsd_dic, locs, arcs):
    """計算リンクの計算関係を得る。
    """
    for el2 in arcs:
...
[ドキュメント]def ReadSchema(inf, is_local, xsd_path, el: ET.Element, xsd_dic: Dict[str, SchemaElement]):
    """スキーマファイルの内容を読む。
    """
    uri, tag_name = split_uri_name(el.tag)
...
[ドキュメント]def get_schema_label_path(inf, ns_uri):
...
[ドキュメント]def readContext(inf, el: ET.Element, parent_tag_name, ctx: Context):
    """コンテキストの情報を得る。
    """
...
[ドキュメント]def makeContextNode(inf, ctx: Context):
...
[ドキュメント]def make_local_ns_dic(inf, path):
    """名前空間の接頭辞とURIの辞書を作る。
    """
    f = open(path)
    for line in f:
...
[ドキュメント]def get_schema_dic(inf, uri) -> Dict[str, SchemaElement]:
...
[ドキュメント]def get_schema_element(inf, uri, tag_name) -> SchemaElement:
    """指定されたURIと名前からスキーマ要素を得る。
    """
...
[ドキュメント]class InlineXbrlParser():
    def __init__(self, inf):

        self.inf = inf
        self.text = None

[ドキュメント]    def handle_context(self, ctx, el, parent_tag_name):
        tag = prefix_tag(el)
...
[ドキュメント]    def handle_unit(self, el):
        tag = prefix_tag(el)
...
[ドキュメント]    def handle_item(self, el, tag):
...
[ドキュメント]    def handle_header(self, el, parent_tag_name):
        tag = prefix_tag(el)
...
[ドキュメント]def read_item(inf, uri, tag_name, context_ref: str, text):
...
[ドキュメント]def process_pending_items(inf):
...
[ドキュメント]def read_xbrl(inf, el: ET.Element):
    """XBRLファイルの内容を読む。
    """
    id, uri, tag_name, text = parseElement(el)
...
[ドキュメント]def readCalcSub(inf, el, xsd_dic, locs, arcs):
    """計算リンクファイルの内容を読む。
    """
    uri, tag_name = split_uri_name(el.tag)
...
[ドキュメント]def read_public_doc(inf, category_name, public_doc, reports):
    """XBRLフォルダー内のファイルを読む。
    """
...
[ドキュメント]def make_public_docs_list(cpu_count, company_dic):
...
[ドキュメント]def readXbrlThread(cpu_count, cpu_id, edinet_code_dic, progress, company_dic):
    """スレッドのメイン処理
    """
    global current_inf
